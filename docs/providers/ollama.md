---
summary: "Run FreeClaw with Ollama (local LLM runtime)"
read_when:
  - You want to run FreeClaw with local models via Ollama
...
Ollama is a local LLM runtime that makes it easy to run open-source models on your machine. FreeClaw integrates with Ollama's OpenAI-compatible API and can **auto-discover tool-capable models** when you opt in with `OLLAMA_API_KEY` (or an auth profile) and do not define an explicit `models.providers.ollama` entry.
...
3. Enable Ollama for FreeClaw (any value works; Ollama doesn't require a real key):
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama`, FreeClaw discovers models from the local Ollama instance at `http://127.0.0.1:11434`:
...
If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and FreeClaw will fill it for availability checks.
...
For auto-discovered models, FreeClaw uses the context window reported by Ollama when available, otherwise it defaults to `8192`. You can override `contextWindow` and `maxTokens` in explicit provider config.
...
If you see garbled responses containing tool names (like `sessions_send`, `memory_get`) or fragmented text when using Ollama models, this is due to an upstream SDK issue with streaming responses. **This is fixed by default** in the latest FreeClaw version by disabling streaming for Ollama models.
