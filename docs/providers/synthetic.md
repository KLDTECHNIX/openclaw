---
summary: "Use Synthetic's Anthropic-compatible API in FreeClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. FreeClaw registers it as the
...
Note: FreeClaw's Anthropic client appends `/v1` to the base URL, so use
