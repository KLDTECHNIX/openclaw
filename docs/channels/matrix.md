...
Matrix is an open, decentralized messaging protocol. FreeClaw connects as a Matrix **user**
...
```bash
freeclaw plugins install @openclaw/matrix
```

Local checkout (when running from a git repo):

```bash
freeclaw plugins install ./extensions/matrix
```
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FreeClaw calls the same
     login endpoint, stores the access token in `~/.freeclaw/credentials/matrix/credentials.json`,
...
- On first connection, FreeClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FreeClaw logs a warning.
...
`~/.freeclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, FreeClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
