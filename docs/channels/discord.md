...
- Talk to FreeClaw via Discord DMs or guild channels.
...
3. Configure FreeClaw with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
This is the “Discord Developer Portal” setup for running FreeClaw in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents FreeClaw needs
...
Discord uses numeric ids everywhere; FreeClaw config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, FreeClaw treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
When enabled, FreeClaw uses the member identity for allowlists and labels the
...
- On startup, FreeClaw resolves channel/user names in allowlists to IDs (when the bot can search members)
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; FreeClaw enforces allowlists on execution and replies “not authorized”.
