...
你也可以设置 `FREECLAW_VERBOSE=1` 代替标志。
...
### `docs.freeclaw.ai` 显示 SSL 错误（Comcast/Xfinity）

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 阻止 `docs.freeclaw.ai`。
禁用 Advanced Security 或将 `docs.freeclaw.ai` 添加到允许列表，然后重试。
