...
summary: 使用设备流从 FreeClaw 登录 GitHub Copilot
...
## 在 FreeClaw 中使用 Copilot 的两种方式

### 1）内置 GitHub Copilot 提供商（`github-copilot`）

使用原生设备登录流程获取 GitHub 令牌，然后在 FreeClaw 运行时将其兑换为 Copilot API 令牌。这是**默认**且最简单的方式，因为它不需要 VS Code。
...
- 登录会将 GitHub 令牌存储在认证配置文件中，并在 FreeClaw 运行时将其兑换为 Copilot API 令牌。
