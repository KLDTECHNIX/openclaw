---
read_when:
  - 你想在 FreeClaw 中使用 OpenAI 模型
  - 你想使用 Codex 订阅认证而非 API 密钥
summary: 在 FreeClaw 中通过 API 密钥或 Codex 订阅使用 OpenAI
title: OpenAI
x-i18n:
...
```bash
freeclaw onboard --auth-choice openai-api-key
# 或非交互式
freeclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
```bash
# 在向导中运行 Codex OAuth
freeclaw onboard --auth-choice openai-codex
