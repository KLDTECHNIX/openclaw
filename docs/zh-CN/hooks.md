...
Hooks 提供了一个可扩展的事件驱动系统，用于响应智能体命令和事件自动执行操作。Hooks 从目录中自动发现，可以通过 CLI 命令管理，类似于 FreeClaw 中 Skills 的工作方式。
...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 FreeClaw 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 助手命令。
...
- **💾 session-memory**：当你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.freeclaw/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.freeclaw/logs/commands.log`
...
2. **托管 hooks**：`~/.freeclaw/hooks/`（用户安装，跨工作区共享）
3. **捆绑 hooks**：`<openclaw>/dist/hooks/bundled/`（随 FreeClaw 附带）
...
Hook 包可以附带依赖；它们将安装在 `~/.freeclaw/hooks/<id>` 下。
...
这些 hooks 不是事件流监听器；它们让插件在 FreeClaw 持久化工具结果之前同步调整它们。
...
- **托管 hooks**（`~/.freeclaw/hooks/`）：跨工作区共享
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.freeclaw/workspace`）
...
- [捆绑 Hooks README](https://github.com/freeclaw/freeclaw/tree/main/src/hooks/bundled)
