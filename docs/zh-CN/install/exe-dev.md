...
summary: 在 exe.dev 上运行 FreeClaw Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
提示：保持此 VM **有状态**。OpenClaw 在 `~/.freeclaw/` 和 `~/.freeclaw/workspace/` 下存储状态。
...
## 4) 设置 nginx 将 FreeClaw 代理到端口 8000
