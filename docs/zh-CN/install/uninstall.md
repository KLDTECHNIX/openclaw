...
如果你将 `FREECLAW_CONFIG_PATH` 设置为状态目录外的自定义位置，也请删除该文件。
...
- 如果你使用了配置文件（`--profile` / `FREECLAW_PROFILE`），对每个状态目录重复步骤 3（默认为 `~/.freeclaw-<profile>`）。
...
默认任务名称是 `FreeClaw Gateway`（或 `FreeClaw Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了 `https://freeclaw.ai/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g freeclaw@latest` 安装的。
使用 `npm rm -g openclaw` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。

### 源码检出（git clone）

如果你从仓库检出运行（`git clone` + `openclaw ...` / `bun run freeclaw ...`）：
