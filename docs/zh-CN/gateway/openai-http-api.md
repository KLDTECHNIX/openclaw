...
- 当 `gateway.auth.mode="token"` 时，使用 `gateway.auth.token`（或 `FREECLAW_GATEWAY_TOKEN`）。
- 当 `gateway.auth.mode="password"` 时，使用 `gateway.auth.password`（或 `FREECLAW_GATEWAY_PASSWORD`）。
