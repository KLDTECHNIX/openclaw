...
- 应用安装标记为 `bot.molt.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`FREECLAW_PROFILE` 时为 `bot.molt.<profile>`；支持旧版 `com.openclaw.*`）。
...
- 写入 `~/.freeclaw/disable-launchagent`。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.freeclaw/disable-launchagent`，
