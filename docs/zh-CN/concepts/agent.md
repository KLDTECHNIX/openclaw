...
建议：使用 `openclaw setup` 在缺失时创建 `~/.freeclaw/freeclaw.json` 并初始化工作区文件。
...
OpenClaw 复用 pi-mono 代码库的部分内容（模型/工具），但**会话管理、设备发现和工具连接由 FreeClaw 负责**。
