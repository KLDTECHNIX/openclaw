...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.freeclaw/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.freeclaw/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.freeclaw`（或 `FREECLAW_STATE_DIR`）
- 工作区：`~/.freeclaw/workspace`（或 `~/.freeclaw/workspace-<agentId>`）
- 智能体目录：`~/.freeclaw/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
...
- 工作区默认为 `~/.freeclaw/workspace`（或当设置了 `FREECLAW_PROFILE` 时为 `~/.freeclaw/workspace-<profile>`）。
- 状态默认为 `~/.freeclaw/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.freeclaw/credentials/whatsapp/personal
