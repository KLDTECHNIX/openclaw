...
read_when:
  ...
  - 你在编写提供商认证或部署环境的文档
summary: FreeClaw 从哪里加载环境变量以及优先级顺序
title: 环境变量
x-i18n:
...
3. **全局 `.env`** 位于 `~/.freeclaw/.env`（即 `$FREECLAW_STATE_DIR/.env`；不覆盖）。
4. **配置 `env` 块** 位于 `~/.freeclaw/freeclaw.json`（仅在缺失时应用）。
5. **可选的登录 shell 导入**（`env.shellEnv.enabled` 或 `FREECLAW_LOAD_SHELL_ENV=1`），仅对缺失的预期键名应用。
