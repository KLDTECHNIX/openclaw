...
- **批准**：通过 `~/.freeclaw/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.freeclaw/freeclaw.json`）。
- `openclaw node run` 读取 `FREECLAW_GATEWAY_TOKEN` 进行认证。
...
- 在 `openclaw node run` / `openclaw node install` 上使用 `--display-name`（持久化在节点上的 `~/.freeclaw/node.json` 中）。
...
批准存储在节点主机的 `~/.freeclaw/exec-approvals.json` 中。
...
freeclaw nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.freeclaw/exec-approvals.json`）。
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.freeclaw/node.json` 中。
- Exec 批准通过 `~/.freeclaw/exec-approvals.json` 在本地执行
  （参见 [Exec 批准](/tools/exec-approvals)）。
- 在 macOS 上，当配套应用 exec 主机可达时，无头节点主机优先使用它，
  如果应用不可用则回退到本地执行。设置 `FREECLAW_NODE_EXEC_HOST=app` 要求
  使用应用，或设置 `FREECLAW_NODE_EXEC_FALLBACK=0` 禁用回退。
