...
新手引导向导是在 macOS、Linux 或 Windows（通过 WSL2；强烈推荐）上设置 FreeClaw 的**推荐**方式。
...
1. **现有配置检测**
   - 如果 `~/.freeclaw/freeclaw.json` 存在，选择**保留 / 修改 / 重置**。
...
2. **模型/认证**
   ...
   - **OpenAI API 密钥**：如果存在则使用 `OPENAI_API_KEY`，否则提示输入密钥，然后保存到 `~/.freeclaw/.env` 以便 launchd 可以读取。
...
- OAuth 凭证存储在 `~/.freeclaw/credentials/oauth.json`；认证配置文件存储在 `~/.freeclaw/agents/<agentId>/agent/auth-profiles.json`（API 密钥 + OAuth）。
- 更多详情：[/concepts/oauth](/concepts/oauth)

3. **工作区**
   - 默认 `~/.freeclaw/workspace`（可配置）。
   - 为智能体引导仪式播种所需的工作区文件。
...
- 默认工作区遵循 `~/.freeclaw/workspace-<agentId>`。
...
- 下载适当的发布资源。
- 存储在 `~/.freeclaw/tools/signal-cli/<version>/` 下。
...
WhatsApp 凭证存储在 `~/.freeclaw/credentials/whatsapp/<accountId>/` 下。
会话存储在 `~/.freeclaw/agents/<agentId>/sessions/` 下。
