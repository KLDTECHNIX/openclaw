...
read_when:
  ...
  - 你想了解浏览器接管的安全影响
summary: Chrome 扩展：让 FreeClaw 驱动你现有的 Chrome 标签页
title: Chrome 扩展
x-i18n:
...
OpenClaw Chrome 扩展让智能体控制你**现有的 Chrome 标签页**（你的正常 Chrome 窗口），而不是启动一个单独的 freeclaw 管理的 Chrome 配置文件。
...
然后 FreeClaw 通过正常的 `browser` 工具界面控制附加的标签页（选择正确的配置文件）。
...
扩展作为静态文件包含在 FreeClaw 发布版（npm 包）中。没有单独的"构建"步骤。

升级 FreeClaw 后：

- 重新运行 `openclaw browser extension install` 以刷新 FreeClaw 状态目录下的已安装文件。
...
CLI 有意**不**打印 `node_modules` 路径。始终先运行 `openclaw browser extension install` 将扩展复制到 FreeClaw 状态目录下的稳定位置。
