...
It works anywhere FreeClaw can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, FreeClaw defaults
...
TTS config lives under `messages.tts` in `freeclaw.json`.
...
- If `provider` is **unset**, FreeClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.freeclaw/settings/tts.json`, override with `FREECLAW_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, FreeClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, FreeClaw:
...
Discord note: `/tts` is a built-in Discord command, so FreeClaw registers
