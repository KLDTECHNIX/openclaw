...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.freeclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.freeclaw/node.json` (node id + pairing token).
...
Path: `~/.freeclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.freeclaw/exec-approvals.sock",
...
- Unix socket at `~/.freeclaw/exec-approvals.sock` (0600).
