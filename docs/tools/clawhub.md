...
- A public registry for FreeClaw skills.
...
4. Users browse, download, and install skills in FreeClaw.
...
If you want to add new capabilities to your FreeClaw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new FreeClaw session so it picks up the new skill.
...
By default, the CLI installs skills into `./skills` under your current working directory. If a FreeClaw workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). FreeClaw loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.freeclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches FreeClaw how to perform a
...
Interested in becoming a moderator? Ask in the FreeClaw Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to FreeClaw workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/openclaw/skills` and `~/.freeclaw/skills`). This is designed to find older skill installs without extra flags.
