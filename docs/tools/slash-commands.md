...
- `commands.config` (default `false`) enables `/config` (reads/writes `freeclaw.json`).
...
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from FreeClaw session logs.
...
- Overrides apply immediately to new config reads, but do **not** write to `freeclaw.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`freeclaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
