---
summary: "Use Amazon Bedrock (Converse API) models with FreeClaw"
read_when:
...
If AWS credentials are detected, FreeClaw can automatically discover Bedrock
...
When running FreeClaw on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, FreeClaw's credential detection currently only checks for environment
...
freeclaw config set models.bedrockDiscovery.enabled true
freeclaw config set models.bedrockDiscovery.region us-east-1
...
- FreeClaw surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
