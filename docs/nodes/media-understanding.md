...
- If `<capability>.enabled: true` but no models are configured, FreeClaw tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, FreeClaw auto-detects in this order and **stops at the first
working option**:
...
lists, FreeClaw can infer defaults:
...
## Provider support matrix (FreeClaw integrations)
