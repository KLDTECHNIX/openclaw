...
- During heartbeat runs, FreeClaw treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, FreeClaw skips the heartbeat run entirely (no model call).
