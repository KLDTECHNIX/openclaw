...
- `funnel`: Public HTTPS via `tailscale funnel`. FreeClaw requires a shared password.
...
- `token` (default when `FREECLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `FREECLAW_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. FreeClaw verifies
...
Prefer `FREECLAW_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
freeclaw gateway --tailscale serve
freeclaw gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want FreeClaw to undo `tailscale serve`
