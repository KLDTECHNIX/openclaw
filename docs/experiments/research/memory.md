...
read_when:
  - Designing workspace memory (~/.freeclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep FreeClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.freeclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.freeclaw/workspace` as canonical human-readable memory.
...
```
~/.freeclaw/workspace/.memory/index.sqlite
```
...
### Why integrate into FreeClaw?

- FreeClaw already knows:
...
Pragmatic take for `~/.freeclaw/workspace`:
