...
When you **explicitly** switch channels with `--channel`, FreeClaw also aligns
the install method:

- `dev` ensures a git checkout (default `~/openclaw`, override with `FREECLAW_GIT_DIR`),
...
When you switch channels with `openclaw update`, FreeClaw also syncs plugin sources:
