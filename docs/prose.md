---
summary: "OpenProse: .prose workflows, slash commands, and state in FreeClaw"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In FreeClaw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
freeclaw plugins enable open-prose
```
...
OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses FreeClaw tools under the hood.
...
## FreeClaw runtime mapping

OpenProse programs map to FreeClaw primitives:

| OpenProse concept         | FreeClaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use FreeClaw tool allowlists and approval gates to control side effects.
