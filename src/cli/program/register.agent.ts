...
${formatHelpExamples([
  ['freeclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['freeclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['freeclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.freeclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.freeclaw.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "freeclaw agents set-identity --identity-file ~/.freeclaw/workspace/IDENTITY.md --agent main",
