...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "freeclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      FREECLAW_NO_RESPAWN: "1",
      FREECLAW_STATE_DIR: stateDir,
      FREECLAW_SKIP_CHANNELS: "1",
      FREECLAW_SKIP_GMAIL_WATCHER: "1",
      FREECLAW_SKIP_CRON: "1",
      FREECLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      FREECLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "freeclaw-entry-bootstrap.mjs");
