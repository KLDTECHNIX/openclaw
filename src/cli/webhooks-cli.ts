...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.freeclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FreeClaw hooks")
    ...
    .option("--hook-url <url>", "FreeClaw hook URL")
    .option("--hook-token <token>", "FreeClaw hook token")
