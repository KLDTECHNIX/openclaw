...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.freeclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.FREECLAW_PROFILE = profile;

  const stateDir = env.FREECLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.FREECLAW_STATE_DIR?.trim()) {
    env.FREECLAW_STATE_DIR = stateDir;
  }

  if (!env.FREECLAW_CONFIG_PATH?.trim()) {
    env.FREECLAW_CONFIG_PATH = path.join(stateDir, "freeclaw.json");
  }

  if (profile === "dev" && !env.FREECLAW_GATEWAY_PORT?.trim()) {
    env.FREECLAW_GATEWAY_PORT = "19001";
  }
}
