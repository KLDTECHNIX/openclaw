...
import {
  ...
  DEFAULT_FREECLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_FREECLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "freeclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_FREECLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_FREECLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an FreeClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for freeclaw/freeclaw CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_FREECLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "freeclaw";
