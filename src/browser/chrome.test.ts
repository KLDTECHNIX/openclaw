...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "freeclaw-chrome-test-"));
    try {
      decorateOpenClawProfile(userDataDir, { color: DEFAULT_FREECLAW_BROWSER_COLOR });
      ...
      expect(def.name).toBe(DEFAULT_FREECLAW_BROWSER_PROFILE_NAME);
      expect(def.shortcut_name).toBe(DEFAULT_FREECLAW_BROWSER_PROFILE_NAME);
      ...
      const marker = await fsp.readFile(
        path.join(userDataDir, ".freeclaw-profile-decorated"),
        "utf-8",
      );
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    try {
      ...
      expect(profile.name).toBe(DEFAULT_FREECLAW_BROWSER_PROFILE_NAME);
    } finally {
      await fsp.rm(userDataDir, { recursive: true, force: true });
    }
  });
