...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".freeclaw-profile-decorated");
}
...
export function decorateOpenClawProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_FREECLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FREECLAW_BROWSER_COLOR).toUpperCase();
