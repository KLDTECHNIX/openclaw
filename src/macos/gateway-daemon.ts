...
declare const __FREECLAW_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __FREECLAW_VERSION__ === "string" && __FREECLAW_VERSION__) ||
  process.env.FREECLAW_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `freeclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.FREECLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.FREECLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[freeclaw] Gateway daemon failed:",
