...
const execHostEnforced = process.env.FREECLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.FREECLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureFreeClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
