...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-update-"));
    await fs.writeFile(path.join(tempDir, "freeclaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "freeclaw", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "freeclaw.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "freeclaw", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "freeclaw");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g freeclaw@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
    ...
    expect(calls.some((call) => call === "npm i -g freeclaw@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g freeclaw@beta")).toBe(true);
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "freeclaw");
    const staleDir = path.join(nodeModules, ".freeclaw-stale");
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "freeclaw");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "freeclaw", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g freeclaw@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "freeclaw", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g freeclaw@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a freeclaw checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-freeclaw-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when freeclaw.mjs is missing", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "freeclaw", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
    ...
    await fs.rm(path.join(tempDir, "freeclaw.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("freeclaw doctor entry");
  });
  ...
  it("repairs UI assets when doctor run removes control-ui files", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "freeclaw.mjs")} doctor --non-interactive`
      ) {
