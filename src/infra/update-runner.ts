...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "freeclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("freeclaw doctor", doctorArgv, gitRoot, { FREECLAW_UPDATE_IN_PROGRESS: "1" }),
    );
