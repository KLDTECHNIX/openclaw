...
const prevBundledDir = process.env.FREECLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `freeclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "freeclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.FREECLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.FREECLAW_BUNDLED_PLUGINS_DIR = bundledDir;
