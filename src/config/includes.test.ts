...
const ETC_FREECLAW_DIR = path.join(ROOT_DIR, "etc", "freeclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "freeclaw.json");
...
function etcOpenClawPath(...parts: string[]) {
  return path.join(ETC_FREECLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "freeclaw.json"))).toEqual({
      shared: true,
    });
  });
});
