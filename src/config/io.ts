...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "FREECLAW_GATEWAY_TOKEN",
  "FREECLAW_GATEWAY_PASSWORD",
];
...
function warnIfConfigFromFuture(cfg: OpenClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FreeClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
// module scope. `FREECLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FREECLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FREECLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
