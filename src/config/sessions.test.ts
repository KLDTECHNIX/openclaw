...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-freeclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-freeclaw#general");
  });
  ...
  it("derives session transcripts dir from FREECLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FREECLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.FREECLAW_STATE_DIR = "/custom/state";
    try {
