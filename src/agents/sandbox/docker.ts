...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "freeclaw.sandbox=1");
  args.push("--label", `freeclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `freeclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `freeclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("freeclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`freeclaw sandbox recreate --session ${params.sessionKey}`);
  }
