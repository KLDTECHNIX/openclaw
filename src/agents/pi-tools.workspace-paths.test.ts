...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("freeclaw-ws-", async (workspaceDir) => {
      await withTempDir("freeclaw-cwd-", async (otherDir) => {
    ...
    await withTempDir("freeclaw-ws-", async (workspaceDir) => {
      await withTempDir("freeclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello freeclaw");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("freeclaw-ws-", async (workspaceDir) => {
      await withTempDir("freeclaw-override-", async (overrideDir) => {
        const tools = createOpenClawCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("freeclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("freeclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
