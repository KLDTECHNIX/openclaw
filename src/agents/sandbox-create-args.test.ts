...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "freeclaw-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "freeclaw-sbx-test",
      cfg,
      scopeKey: "main",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "freeclaw.sandbox=1",
        "--label",
        "freeclaw.sessionKey=main",
        "--label",
        "freeclaw.createdAtMs=1700000000000",
        "--label",
        "freeclaw.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
