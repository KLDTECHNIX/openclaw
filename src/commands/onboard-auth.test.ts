...
const requireAgentDir = () => {
  const agentDir = process.env.FREECLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FREECLAW_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  const previousAgentDir = process.env.FREECLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.FREECLAW_AGENT_DIR;
    } else {
      process.env.FREECLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.FREECLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under FREECLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-oauth-"));
    process.env.FREECLAW_STATE_DIR = tempStateDir;
    process.env.FREECLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FREECLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FREECLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-minimax-"));
    process.env.FREECLAW_STATE_DIR = tempStateDir;
    process.env.FREECLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FREECLAW_AGENT_DIR;
