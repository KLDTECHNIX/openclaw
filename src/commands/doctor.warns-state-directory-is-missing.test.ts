...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.FREECLAW_STATE_DIR;
  originalUpdateInProgress = process.env.FREECLAW_UPDATE_IN_PROGRESS;
  process.env.FREECLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "freeclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.FREECLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.FREECLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.FREECLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "freeclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.FREECLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when FREECLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/freeclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.FREECLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
