...
beforeAll(() => {
  previousProfile = process.env.FREECLAW_PROFILE;
...
vi.mock("../infra/freeclaw-root.js", () => ({
  resolveOpenClawPackageRoot: vi.fn().mockResolvedValue("/tmp/freeclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/freeclaw/pnpm-lock.yaml",
      markerPath: "/tmp/freeclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("FreeClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("freeclaw status --all") ||
          l.includes("freeclaw --profile isolated status --all") ||
          l.includes("freeclaw status --all") ||
          l.includes("freeclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FREECLAW_GATEWAY_TOKEN = "abcd1234";
    try {
