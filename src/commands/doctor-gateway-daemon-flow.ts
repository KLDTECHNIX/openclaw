...
import { renderRcdUnavailableHints } from "../daemon/rcd-hints.js";
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (!loaded) {
    const rcdAvailable = await isRcdServiceAvailable().catch(() => false);
    if (!rcdAvailable) {
      note(renderRcdUnavailableHints().join("\n"), "Gateway");
      return;
    }
    note("Gateway service not installed.", "Gateway");
    if (params.cfg.gateway?.mode !== "remote") {
      const install = await params.prompter.confirmSkipInNonInteractive({
        message: "Install gateway rc.d service now?",
        initialValue: true,
      });
      if (install) {
        ...
        const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
          env: process.env,
          port,
          token: params.cfg.gateway?.auth?.token ?? process.env.FREECLAW_GATEWAY_TOKEN,
