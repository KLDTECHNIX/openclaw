...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.FREECLAW_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.FREECLAW_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "freeclaw");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.FREECLAW_STATE_DIR = stateDir;
    process.env.FREECLAW_CONFIG_PATH = path.join(stateDir, "freeclaw.json");
