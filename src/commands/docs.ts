...
const SEARCH_TOOL = "https://docs.freeclaw.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.freeclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('freeclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.freeclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('freeclaw docs "your query"')}`);
    }
    return;
  }
