...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FREECLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FREECLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FREECLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("FREECLAW_GATEWAY_PASSWORD");
  });
