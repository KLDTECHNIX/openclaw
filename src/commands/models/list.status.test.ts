...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenClawAgentDir: vi.fn().mockReturnValue("/tmp/freeclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/freeclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/freeclaw-agent/auth-profiles.json"),
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/freeclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/freeclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/freeclaw-agent-custom");
