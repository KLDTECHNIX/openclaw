...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("freeclaw models auth setup-token")}\` or \`${formatCliCommand("freeclaw configure")}\`.`
          : `Run \`${formatCliCommand("freeclaw configure")}\` or set an API key env var.`;
