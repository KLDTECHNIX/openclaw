...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "FREECLAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.FREECLAW_STATE_DIR = path.join(home, ".freeclaw");
    ...
    await fs.mkdir(path.join(home, ".freeclaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.freeclaw${path.sep}media`);
