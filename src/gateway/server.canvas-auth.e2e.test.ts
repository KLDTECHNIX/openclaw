...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.FREECLAW_CONFIG_PATH;
  const prevDisableCache = process.env.FREECLAW_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "freeclaw-canvas-auth-test-"));
  const configPath = path.join(dir, "freeclaw.json");

  process.env.FREECLAW_CONFIG_PATH = configPath;
  process.env.FREECLAW_DISABLE_CONFIG_CACHE = "1";
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.FREECLAW_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.FREECLAW_DISABLE_CONFIG_CACHE;
    } else {
      process.env.FREECLAW_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
