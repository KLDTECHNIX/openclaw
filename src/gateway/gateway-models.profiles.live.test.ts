...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.FREECLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.FREECLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.FREECLAW_LIVE_GATEWAY_PROVIDERS);
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.FREECLAW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.FREECLAW_STATE_DIR,
  };
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-live-state-"));
  process.env.FREECLAW_STATE_DIR = tempStateDir;
  ...
  process.env.FREECLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.freeclaw-live-tool-probe.${nonceA}.txt`);
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-live-"));
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "FreeClaw live tool probe (local, safe): " +
    ...
    process.env.FREECLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.FREECLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.FREECLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.freeclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
