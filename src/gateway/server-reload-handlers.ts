  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.FREECLAW_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (FREECLAW_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (FREECLAW_SKIP_CHANNELS=1 or FREECLAW_SKIP_PROVIDERS=1)",
        );
      } else {
