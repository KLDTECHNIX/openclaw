...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.FREECLAW_STATE_DIR;
  previousConfigPath = process.env.FREECLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.FREECLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.FREECLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.FREECLAW_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.FREECLAW_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.FREECLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "freeclaw-test-no-bundled-extensions")
    : "freeclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "freeclaw-test-"))
    : path.join(tempHome, ".freeclaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.FREECLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.FREECLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.FREECLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.FREECLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.FREECLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.FREECLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.FREECLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.FREECLAW_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.FREECLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.FREECLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FREECLAW_GATEWAY_TOKEN = fallbackToken;
  }
