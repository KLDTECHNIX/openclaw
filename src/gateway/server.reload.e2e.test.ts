...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.FREECLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.FREECLAW_SKIP_GMAIL_WATCHER;
    process.env.FREECLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.FREECLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.FREECLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.FREECLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
