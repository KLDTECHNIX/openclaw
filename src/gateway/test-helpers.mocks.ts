...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `freeclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.FREECLAW_CONFIG_PATH = path.join(root, "freeclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "freeclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "freeclaw-gateway-test"),
        ...fileDefaults,
