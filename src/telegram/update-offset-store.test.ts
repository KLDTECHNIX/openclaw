...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.FREECLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "freeclaw-telegram-"));
  process.env.FREECLAW_STATE_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (previous === undefined) {
      ...
      process.env.FREECLAW_STATE_DIR = previous;
    }
