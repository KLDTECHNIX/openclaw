...
`;
    ...
    expect(parsed.freeclaw.emoji).toBe("💾");
    expect(parsed.freeclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.freeclaw.emoji).toBe("📝");
    expect(parsed.freeclaw.events).toEqual(["command"]);
    expect(parsed.freeclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.freeclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"freeclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"freeclaw": {"events": ["test"]}}');
  });
...
describe("resolveOpenClawMetadata", () => {
  it("extracts freeclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
  ...
  it("returns undefined when freeclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    ...
    expect(result?.install?.[1].package).toBe("@freeclaw/hook");
  });
...
`;
    ...
    const freeclaw = resolveOpenClawMetadata(frontmatter);
    expect(freeclaw).toBeDefined();
    expect(freeclaw?.emoji).toBe("💾");
    expect(freeclaw?.events).toEqual(["command:new"]);
    expect(freeclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(freeclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const freeclaw = resolveOpenClawMetadata(frontmatter);
    expect(freeclaw?.emoji).toBe("disk");
    expect(freeclaw?.events).toEqual(["command:new"]);
  });
});
