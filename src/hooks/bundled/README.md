...
This directory contains hooks that ship with FreeClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
metadata:
  { "freeclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.freeclaw/freeclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'freeclaw.*gateway' && pnpm freeclaw gateway`
3. Enable the hook: `freeclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.freeclaw.ai/hooks
