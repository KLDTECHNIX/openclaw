...
homepage: https://docs.freeclaw.ai/hooks#command-logger
metadata:
  {
    "freeclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.freeclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.freeclaw/logs/commands.log ~/.freeclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/freeclaw`:
   ```
   /home/username/.freeclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
