...
describe("buildPairingReply", () => {
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect FREECLAW_PROFILE when set (most tests run with isolated profile)
      const commandRe = new RegExp(
        `(?:freeclaw|freeclaw) --profile isolated pairing approve ${testCase.channel} <code>`,
      );
