...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.FREECLAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.FREECLAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.FREECLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[freeclaw] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[freeclaw] ${parsed.error}`);
    process.exit(2);
  }
