...
`openclaw.invoke --each`) without adding new FreeClaw code per workflow.
...
This extension depends on FreeClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FreeClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.freeclaw/extensions` as a standalone plugin directory.
