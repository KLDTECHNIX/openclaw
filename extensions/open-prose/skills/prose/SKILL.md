...
- **Task tool** in the upstream spec == FreeClaw `sessions_spawn`
- **File I/O** == FreeClaw `read`/`write`
- **Remote fetch** == FreeClaw `web_fetch` (or `exec` with curl when POST is required)
